...
// 人一阶单属性玩家模板基准值（V1 旧模板基准）。
// 注意：下列常量源自 V1 旧衍生公式（气血=精×100、攻击=气×12、防御=精×10、速度=精×5），
// 而人物系统已升级为 V2 公式（character/calc.go CalcDerivedAttrs：
// 气血=精×50、体魄=精×5、身法=精×1、功法威力=气×2 等），
// 因此这套怪物基准值已与 V2 人物模板脱钩，仅作为怪物系统内部的相对数值使用。
// TODO: 待数值策划按 V2 公式重算怪物基准值后统一调整（本次评审仅更新注释，常量值不改）。
const (
	BaseHP  = 100 // 基准气血
	BaseATK = 12  // 基准攻击
	BaseDEF = 10  // 基准防御
