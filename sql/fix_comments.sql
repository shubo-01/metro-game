-- 修复注释编码：先设连接字符集为utf8mb4，再执行ALTER
...
ALTER TABLE players
  ...
  MODIFY updated_at DATETIME COMMENT '最后更新时间（移动或升级时刷新）';

ALTER TABLE player_attrs COMMENT='角色属性表：五维显性属性（精气神、气运、悟性）加五行灵力加隐藏属性';
...
ALTER TABLE login_sessions COMMENT='登录会话表：存储JWT双Token（AccessToken和RefreshToken），支持单点登录踢下线';
ALTER TABLE login_sessions
  MODIFY id BIGINT COMMENT '会话记录ID',
  MODIFY account_id BIGINT COMMENT '登录账号ID，对应accounts.id',
  MODIFY player_id BIGINT COMMENT '关联角色ID，登录后选择或创建角色后回填',
  MODIFY platform VARCHAR(20) COMMENT '登录平台：wechat或douyin或app_ios或app_android',
...
ALTER TABLE login_logs COMMENT='登录日志表：记录每次登录尝试（成功或失败），用于安全审计和异常检测';
ALTER TABLE login_logs
  ...
  MODIFY reason VARCHAR(255) COMMENT '失败原因描述（成功时为空），如验证码错误或账号封禁',
